...
def check_for_complete_air(fasta,V,D,J,igBlastPath):
...
def main():
    ...
    '''
    inDir = sys.argv[1]
    outDir = ('/').join(inDir.split('/')[:-1])
    V= sys.argv[5]
    D= sys.argv[6]
    J= sys.argv[7]
    type1=sys.argv[8]
    igBlastPath=sys.argv[9]
    fileList = os.listdir(inDir)
    final=open(outDir+'/'+type1+'.fasta','w')
    faList=[]
    for file in os.listdir(inDir):
        if 'sam' in file and type1 in file:
            faList.append(file.split('.')[0])
    faList=sorted(faList,key=lambda x:int(x.split('_')[1]))
    ...
    for _, inputs in cellDict.items():
        ...
        check_for_complete_air(inDir+'/'+fasta,V,D,J,igBlastPath)
        temp_fastq_reads_full,temp_fastq_reads_partial,temp_fasta_reads,abundance=extract_reads(inDir+'/'+fasta,inDir+'/'+fastq)
    ...
    final.close()
    check_for_complete_air(outDir+'/'+type1+'.fasta',V,D,J,igBlastPath)
    os.system('sort '+outDir+'/'+type1+'.fasta.table.parsed > '+outDir+'/'+type1+'.fasta.table.parsed.sorted')
